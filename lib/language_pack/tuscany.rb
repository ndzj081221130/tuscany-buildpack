...
module LanguagePack
  class Tuscany
    ...
    DEFAULT_MVN_VERSION = "3.0.5".freeze
    def self.use?
    ...
    def install_mvn
      FileUtils.mkdir_p mvn_dir
      mvn_tarball = "#{mvn_dir}/mvn.tar.gz"
      download_mvn mvn_tarball
      
      puts "unpacking MVN to #{mvn_dir}"
      tar_output = run_with_err_output "tar pxzf #{mvn_tarball} -C #{mvn_dir}"

   #   FileUtils.rm_rf jdk_tarball
      unless File.exists?("#{mvn_dir}/bin/mvn")
        puts "Unable to retrieve the MVN"
        puts tar_output
        exit 1
      end
    end
    ...
    def mvn_version
      @mvn_version ||= system_properties["mvn.runtime.version"] || DEFAULT_MVN_VERSION
    end
    ...
    def download_mvn(mvn_tarball)
      puts "download mvn"
      fetched_package_mvn = fetch_mvn_package(mvn_version)  
      FileUtils.mv fetched_package_mvn, mvn_tarball
